...
clock), a spec-first architecture (JSON Schema + semantic invariants with located,
machine-actionable errors), agent-first authoring (an MCP server + agent-facing authoring docs —
no embedded LLM; users bring their own agent), four delivery sinks (files, Kafka, RabbitMQ,
webhooks) with history-to-live streaming continuity at controlled TPS, a per-run realism
report, and a six-surface React studio — all behind one Fastify REST/WebSocket API consumed
identically by the web UI, CLI, and MCP server, shipped as one `docker compose up`. Technical
approach and stack were fixed by user interview (see research.md; agent-first revision
2026-07-13): TypeScript end to end, Fastify, MySQL via Knex (no ORM), BullMQ + Redis, Vite SPA,
Vitest, pnpm monorepo.
...
@dsnp/parquetjs (Parquet), @modelcontextprotocol/sdk (MCP server, streamable-HTTP transport
mounted on the API — no LLM SDKs anywhere in v1)
...
Kafka, and RabbitMQ integration tests; contract tests per REST/WS endpoint and per MCP tool
(1:1 REST parity asserted); React component tests for spec-editor and export surfaces
...
`Math.random`, no wall-clock reads in generation logic); no embedded language model anywhere in
v1 — agent authoring is external via the MCP server + shipped docs, and no language model may
ever generate transactions; truth records immutable (imperfections corrupt delivered
...
**Scale/Scope**: 6 UI surfaces, ~15 REST resources + 2 WS channels + 1 MCP endpoint (~10 tools
mapping 1:1 onto REST), 3 fraud typologies, 4 sinks, 4 imperfection types, 4 scenario templates;
reference scale 200k consumers / 5k merchants / 90 days / low-millions of events per run; 8-week
roadmap with documented de-scope order
...
| 2 | Test gate (test-first) | Engine/validator/API code developed red→green with tests in same PR: golden-master suites (seed+spec → committed reference output), fast-check property tests with tolerance bounds for statistical targets, contract tests per REST endpoint and per MCP tool (1:1 parity asserted), Testcontainers integration tests for Kafka/RabbitMQ/MySQL/webhooks; UI component tests for spec-editor, export surfaces | PASS |
| 3 | Determinism gate | Single seeded RNG entry (pure-rand) with per-partition substreams; d3-random distributions take the seeded source; virtual clock injected, wall-clock banned in engine; no LLM in the product — agent tool definitions confined to `packages/agent-tools`, MCP tools map 1:1 onto REST and only emit/accept specs; golden-master CI job fails on output drift; output-altering changes require declared breaking version | PASS |
...
FR-001); UI/CLI/MCP are pure clients of one Fastify API (Principle IV); `docker compose up`
single-command distribution preserved (MySQL + Redis + app containers only — the MCP server is
an endpoint on the API, not a new service); no embedded LLM at all — agent authoring is external
via MCP + shipped docs (Principle: openness).
...
│   ├── src/
│   │   ├── routes/          # scenarios, specs, runs, streams, sinks, settings, templates,
│   │   │                    # exports, capabilities
│   │   ├── mcp/             # MCP server endpoint (streamable HTTP) over packages/agent-tools
...
│   └── tests/               # contract/ (REST + MCP-tool parity) + integration/ (Testcontainers)
...
│   │   ├── components/      # Monaco spec editor, version diff, charts (Recharts), tickers
│   │   ├── api/             # typed client over contracts (REST + WS)
│   │   └── app/             # shell, routing, theme, capability-gated surfaces
│   └── tests/               # component tests (spec-editor, export controls)
...
│   │                        # merchants, party naming (locale dictionary packs), seasonality,
│   │                        # fraud actors/typologies, imperfection layer, partitioning,
│   │                        # truth-record emission, realism-report computation
│   ├── src/
...
└── agent-tools/             # shared agent tool definitions (names, schemas, descriptions) +
    │                        # authoring-docs source (invariant catalog, worked examples) —
    │                        # single source for the MCP server and any future AI-assist plugin;
    │                        # never touches the engine
    ├── src/
    └── tests/

docs/agent/                  # published agent authoring guide: schema reference, invariant
                             # error codes + remedies, worked example specs (built from
                             # packages/agent-tools sources)
...
`engine`, `sinks`, `agent-tools`) isolate the deterministic core from delivery and from the
agent-integration surface; apps (`api`, `worker`, `web`, `cli`) are thin hosts. The API serves
the built SPA and hosts the MCP endpoint so compose stays at four containers.
